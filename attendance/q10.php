<?php
    ...
    if(isset($_GET['student']) && isset($_GET['classes']) )
{
    $id=$_GET['student'];
...
join bitnami_moodle.mdl_attendance_statuses ats on ats.id = al.statusid
 where u.id='".$id."' and cc.id = '16' and a.id not in('69','68','67','43', '65') and ra.roleid ='5'   
...
join bitnami_moodle.mdl_user u on u.id = al.studentid
join bitnami_moodle.mdl_attendance_statuses ats on ats.id = al.statusid
 where cc.id = '156777' and a.id not in('69','68','67','43', '65') and ra.roleid ='5'
 group by u.firstname, date ORDER by date DESC";
}
