...
{  
  ...
  $myquery = "SELECT `Term` AS `Term`,
       `Class` AS `Class`,
       avg(Marks) AS `avg__finalgrade`
FROM
...
if($classes=='All' && $subject=='All'){
    
}
else if($classes!='All' && $subject=='All')
{
    $myquery=$myquery." WHERE (Class = '".$classes."')";
}
else if($subject!='All' && $classes=='All'){
    $myquery=$myquery." WHERE (Subject = '".$subject."')";

}
else if($subject!='All' && $classes!='All')
{
    $myquery=$myquery." WHERE (Class = '".$classes."') and ( Subject = '".$subject."')";
}



$myquery=$myquery." GROUP BY `Term`,
         `Class` ORDER BY Class,`avg__finalgrade` DESC;
";
